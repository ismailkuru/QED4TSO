...
procedure bakery();
  modifies counter, number, owner, number, owner;
...
implementation bakery()
{
  ...
  Atomic_16:
    atomic  {  // Non-mover
        assert number[tid] > 0;
        havoc j;
        assume (forall t: TID :: {number[t]} 1 <= t && t < Tid && t != tid ==> number[t] == 0 || number[t] > number[tid] || (number[t] == number[tid] && t > tid));
        /* assert counter == 0; [Discharged] */
        counter := counter + 1;

        assert owner == tid;
        /* assert counter == 1; [Discharged] */

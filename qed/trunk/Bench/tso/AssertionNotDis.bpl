...
implementation send(addrFlag: int, addrX: int)
{
  ...
  Call_45:
    atomic  {  // Non-mover
        assert ThreadWriteBuffers[recvThread].head == ThreadWriteBuffers[recvThread].tail ==> Mem[addrFlag] == toSend && ownerMap[addrFlag] == senderThread && Mem[addrX] == toRd && ownerMap[addrX] == senderThread;
        ...
        assert ThreadWriteBuffers[senderThread].tail == ThreadWriteBuffers[senderThread].head;
        havoc Mem;
        havoc newHead;
        ...
        assume Mem[addrFlag] == UpdateMemSEG(Mem, fresh_6.writebuffer, fresh_6.head, newHead, Node_addr, Node_value)[addrFlag];
        assume Mem[addrX] == UpdateMemSEG(Mem, fresh_6.writebuffer, fresh_6.head, newHead, Node_addr, Node_value)[addrX];
        ...
        assume fresh_3.addr == addrX;
        assume fresh_3.value == toRd;
        ...
        havoc Mem;
        havoc newHead1;
        assume fresh_7 == ThreadWriteBuffers[senderThread];
        assume fresh_7.head <= fresh_7.tail;
        assume fresh_7.head <= newHead1 && newHead1 <= fresh_7.tail;
        assume Mem[addrFlag] == UpdateMemSEG(Mem, fresh_7.writebuffer, fresh_7.head, newHead1, Node_addr, Node_value)[addrFlag];
        assume Mem[addrX] == UpdateMemSEG(Mem, fresh_7.writebuffer, fresh_7.head, newHead1, Node_addr, Node_value)[addrX];
        assert ThreadWriteBuffers[recvThread].tail == ThreadWriteBuffers[recvThread].head;
        assert ThreadWriteBuffers[senderThread].head == ThreadWriteBuffers[senderThread].tail ==> Mem[addrX] == toRd;
        assert ThreadWriteBuffers[senderThread].head < ThreadWriteBuffers[senderThread].tail ==> ThreadWriteBuffers[senderThread].writebuffer[ThreadWriteBuffers[senderThread].head].value == toRd;
        assert ThreadWriteBuffers[senderThread].head < ThreadWriteBuffers[senderThread].tail ==> ThreadWriteBuffers[senderThread].writebuffer[ThreadWriteBuffers[senderThread].head].addr == addrX;
        assert ThreadWriteBuffers[senderThread].head < ThreadWriteBuffers[senderThread].tail ==> ThreadWriteBuffers[senderThread].tail == ThreadWriteBuffers[senderThread].head + 1;
        assert Mem[addrFlag] == toSend && ownerMap[addrFlag] == senderThread && ownerMap[addrX] == senderThread && flagLocal == toSend;
        assert senderThread == tid;
        assert true;
        ...
        fresh_5 := fresh_4.writebuffer[fresh_4.tail];
        assume fresh_5.idx == fresh_4.tail;
        assume fresh_5.addr == addrFlag;
        assume fresh_5.value == toRecv;
        fresh_4.tail := fresh_4.tail + 1;
        assert senderThread == tid;
        assert Mem[addrFlag] == toSend && ownerMap[addrFlag] == senderThread && ownerMap[addrX] == senderThread && flagLocal == toSend;
        assert ThreadWriteBuffers[senderThread].head < ThreadWriteBuffers[senderThread].tail;
        ...
        havoc Mem;
        havoc newHead2;
        assume fresh_8 == ThreadWriteBuffers[senderThread];
        assume fresh_8.head <= fresh_8.tail;
        assume fresh_8.head <= newHead2 && newHead2 <= fresh_8.tail;
        assume Mem[addrFlag] == UpdateMemSEG(Mem, fresh_8.writebuffer, fresh_8.head, newHead2, Node_addr, Node_value)[addrFlag];
        assume Mem[addrX] == UpdateMemSEG(Mem, fresh_8.writebuffer, fresh_8.head, newHead2, Node_addr, Node_value)[addrX];
    }
}
...
implementation recieve(addrFlag: int, addrX: int) returns (result: int)
{
  ...
  Call_52:
    atomic  {  // Non-mover
        assert ThreadWriteBuffers[senderThread].head == ThreadWriteBuffers[senderThread].tail ==> Mem[addrFlag] == toRecv && ownerMap[addrFlag] == recvThread && Mem[addrX] == toRd && ownerMap[addrX] == recvThread;
        ...
        if (*)
        {
            assume fresh_3.idx >= fresh_2.head && fresh_3.idx < fresh_2.tail && fresh_2.writebuffer[fresh_3.idx].addr == addrX && (forall nodeRecent: Node :: nodeRecent.idx > fresh_3.idx && nodeRecent.idx <= fresh_2.tail ==> nodeRecent.addr != fresh_3.addr && nodeRecent.addr != addrX);
            ...
            result := fresh_2.writebuffer[fresh_3.idx].value;
        }
        else
        {
        ...
        havoc Mem;
        havoc newHead1;
        assume fresh_7 == ThreadWriteBuffers[recvThread];
        assume fresh_7.head <= fresh_7.tail;
        assume fresh_7.head <= newHead1 && newHead1 <= fresh_7.tail;
        assume Mem[addrFlag] == UpdateMemSEG(Mem, fresh_7.writebuffer, fresh_7.head, newHead1, Node_addr, Node_value)[addrFlag];
        assume Mem[addrX] == UpdateMemSEG(Mem, fresh_7.writebuffer, fresh_7.head, newHead1, Node_addr, Node_value)[addrX];
        ...
        assert ThreadWriteBuffers[recvThread].writebuffer[ThreadWriteBuffers[recvThread].head].addr == addrFlag;
        assert ThreadWriteBuffers[recvThread].writebuffer[ThreadWriteBuffers[recvThread].head].value == toSend;
        assert ThreadWriteBuffers[senderThread].head == ThreadWriteBuffers[senderThread].tail;
        havoc Mem;
        havoc newHead2;
        assume fresh_8 == ThreadWriteBuffers[recvThread];
        assume fresh_8.head <= fresh_8.tail;
        assume fresh_8.head <= newHead2 && newHead2 <= fresh_8.tail;
        assume Mem[addrFlag] == UpdateMemSEG(Mem, fresh_8.writebuffer, fresh_8.head, newHead2, Node_addr, Node_value)[addrFlag];
        assume Mem[addrX] == UpdateMemSEG(Mem, fresh_8.writebuffer, fresh_8.head, newHead2, Node_addr, Node_value)[addrX];

...
const unique INF:int;

invariant (forall i :int :: i< INF);
...
procedure pop() returns(result:int){
		...
		if(headLocal < tailLocal){
			...
			//DRAIN
			call  tailMinLocal := read(TSKQ.tailMin);
			
			//DRAIN
			call tailLocal := read(TSKQ.tail);
		
			if(tailMinLocal > tailLocal){ 
				
				//DRAIN
				call write(TSKQ.tailMin , tailLocal);
			}
			...
			atomic{
				...
				if (headLocal > tailMinLocal)
					call  tailMinLocal := read(TSKQ.tailMin);
					call write (TSKQ.head ,tailMinLocal);
					
				call write (TSKQ.tailMin , INF);
				call headLocal := read(TSKQ.head);
				call tailLocal := read(TSKQ.tail);
			
				if (headLocal <= tailLocal) {
					call result := read(TSKQ.tasks[TSKQ.tail]);
					call write(TSKQ.tasks[tail] , NULL_INT);
				} 
				else {
					call tailLocal := read(TSKQ.tail);
					tailLocal := tailLocal +1 ;//call write(tailLocal , tailLocal+1);
  	...
  	atomic{
		...
		if (headLocal < tailLocal){
		   ...
		   call write(TSKQ.head, headLocal);
			...
			// Bunu boyle yazmamin sebebi sadece resultAddr yi bir address veya handle olarak gormem
			call write(resultAddr, NULL_INT);
			call result := read(resultAddr);
		}

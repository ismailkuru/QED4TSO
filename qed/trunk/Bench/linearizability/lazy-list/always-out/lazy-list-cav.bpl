...
procedure contains(e: int) returns (wasPresent: bool)
{
	...
	// no node with key E exists
	assert ReachBetweenSet(Hnext[qseq[qcount]], curr, Tail)[Tail];
	assert (forall m: Node :: {ReachBetweenSet(Hnext[qseq[qcount]], curr, Tail)[m]} ReachBetweenSet(Hnext[qseq[qcount]], curr, Tail)[m] ==> m.key != E || Hmarked[qseq[qcount]][m]==True); //***

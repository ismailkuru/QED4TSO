...
procedure Add(e: int) returns (res: bool);
  modifies Node_alloc, Node_next, Mutex_owner, Mutex_owner;
...
implementation Add(e: int) returns (res: bool)
{
  ...
  Call_46:
    atomic  {  // Non-mover
        ...
        assume n1.val < e;
        assume n3.val >= e;
        assume IsAlloc(n1.alloc) && IsAlloc(n3.alloc);
        assume n1.marked == False && n3.marked == False ==> ReachBetween(Node_next, Head, n1, n3);
        assume n1.mutex.owner == 0;
        n1.mutex.owner := tid;
        assume n3.mutex.owner == 0;
        n3.mutex.owner := tid;
  ...
  Call_47:
    atomic  {  // Non-mover
        assert n1.mutex.owner == tid;
        n1.mutex.owner := 0;
    }

  Call_48:
    atomic  {  // Non-mover
        assert n3.mutex.owner == tid;
        n3.mutex.owner := 0;
    }
}
...
procedure Remove(e: int) returns (res: bool);
  modifies Node_marked, Node_next, Mutex_owner, Mutex_owner;
    ...
    if (*)
    {
      Call_49_then:
        atomic  {  // Non-mover
            ...
            assume n1.val < e;
            assume n2.val >= e;
            assume IsAlloc(n1.alloc) && IsAlloc(n2.alloc);
            assume n1.marked == False && n2.marked == False ==> ReachBetween(Node_next, Head, n1, n2);
            assume n1.mutex.owner == 0;
            n1.mutex.owner := tid;
            assume n2.mutex.owner == 0;
            n2.mutex.owner := tid;
            assume n1.marked == False;
            assume n2.marked == False;
            assume n1.next == n2;
            ...
            Hver[Hcount] := Hver[Hcount - 1];
            Hver[Hcount][e] := Hver[Hcount][e] + 1;
            /* assert IsAlloc(n2.alloc); [Discharged] */
            /* assert n1.mutex.owner == tid && n2.mutex.owner == tid; [Discharged] */
            n3 := n2.next;
        }

      Atomic_51:
        atomic  {  // Non-mover
            assume n1.val < e && n2.val >= e && IsAlloc(n1.alloc) && IsAlloc(n2.alloc) && n1.marked == False && n2.marked == True && ReachBetween(Node_next, Head, n1, n2) && n1.mutex.owner == tid && n2.mutex.owner == tid && n1.next == n2;

            /* assert n1.mutex.owner == tid && n2.mutex.owner == tid; [Discharged] */
            assume Hnext[Hcount] == Node_next;
            assume Hmarked[Hcount] == Node_marked;
            /* assert IsAlloc(n1.alloc); [Discharged] */
            assert IsAlloc(n3.alloc) || n3 == null;
            /* assert IsAlloc(n2.alloc); [Discharged] */
            assert n1.val < e && e == n2.val;
            assert n1.next == n2 && n2.next == n3;
            assert n3 != null ==> e < n3.val;
            /* assert n2.marked == True; [Discharged] */
            n1.next := n3;
            Hcount := Hcount + 1;
            Hnext[Hcount] := Node_next;
            Hmarked[Hcount] := Node_marked;

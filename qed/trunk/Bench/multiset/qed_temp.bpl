...
procedure Insert(x: ref) returns (r: bool, i: int);
  modifies Cell_vld, Mutex_owner, Mutex_owner, Cell_elt;



implementation Insert(x: ref) returns (r: bool, i: int)
{
  var ex: Exception;
  var fresh_0: int;
  var fresh_1: bool;
...
procedure InsertPair(x: ref, y: ref) returns (r: bool, i: int, j: int);
  modifies Cell_elt, Cell_vld, Mutex_owner, Mutex_owner;



implementation InsertPair(x: ref, y: ref) returns (r: bool, i: int, j: int)
{
  var ex: Exception;
  var fresh_0: int;
  var fresh_1: bool;
...
procedure Delete(x: ref) returns (r: bool, i: int);
  modifies Cell_elt, Cell_vld, Mutex_owner;



implementation Delete(x: ref) returns (r: bool, i: int)
{
  var ex: Exception;
  var guard_1: bool;
  ...
  Atomic_39:
    atomic  {  // Non-mover
        ...
        if (i < N)
        {
            ...
            assume M[i].lck.owner == 0;
            M[i].lck.owner := tid;
            guard_1 := M[i].elt == x && M[i].vld == True;
            if (guard_1)
            {
                M[i].elt := null;
                M[i].vld := False;

...
implementation Sync_tryRelease(this: NonfairSync, releases: int) returns (result: bool)
{
  ...
  Call_108:
    atomic { // Non-mover
        ...
        {
            // assert lockedBy[this] != tid; [Discharged]
        ...
        // assert this.exclusiveOwnerThread == Threads[tid]; [Discharged]
        // assert lockedBy[this] == tid; [Discharged]
        result := false;
        if (this.state == 0)
        {
...
implementation Sync_nonfairTryAcquire(this: NonfairSync, acquires: int) returns (result: bool)
{
  ...
  Atomic_117:
    atomic { // Non-mover
        ...
        {
            // assert lockedBy[this] == tid || lockedBy[this] == 0; [Discharged]
            c := this.state;
        }
        else
        {
  ...
  Atomic_171:
    atomic { // Non-mover
        ...
        else
        {
            ...
            {
                // assert lockedBy[this] == tid; [Discharged]
                // assert c == this.state; [Discharged]
                // assert this.state > 0; [Discharged]
                havoc nextc;
                if (this.state + 1 < 0)
                {
...
invariant (forall queue: NonfairSync :: lockedBy[queue] == tid <==> queue.state > 0);

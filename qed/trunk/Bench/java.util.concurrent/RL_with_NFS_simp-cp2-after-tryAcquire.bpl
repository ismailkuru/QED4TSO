...
procedure {:ispublic false} AQS_addWaiter(this: NonfairSync, mode: Node) returns (result: Node);
  modifies Node_alloc, owner, Node_nextWaiter, Node_thread, Node_next, Node_prev, Node_waitStatus, con, NonfairSync_tail, NonfairSync_head;
...
implementation AQS_addWaiter(this: NonfairSync, mode: Node) returns (result: Node)
{
  ...
  var fresh_0: Node;
  var fresh_1: Node;
  var abst_NonfairSync_tail_0: [NonfairSync]Node;
  ...
  Atomic_21:
    atomic { // Non-mover
        ...
        node.thread := Threads[tid];
        node.next := NULL_NODE;
        node.prev := NULL_NODE;
        node.waitStatus := NO_WAIT_STATUS;
        con[node] := True;
        havoc abst_NonfairSync_tail_0;
        pred := abst_NonfairSync_tail_0[this];
        node.prev := pred;
        havoc dummy;
        if (this.tail == pred && this.tail != NULL_NODE)
        {
            this.tail := node;

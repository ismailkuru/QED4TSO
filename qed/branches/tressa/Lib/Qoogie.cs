...
namespace QED
{
    ...
    public class Qoogie2Boogie
    {
        ...
        {
            this.rctx = Qoogie.ResolveStmt(procState, stmtList);
        ...
        private void TranslateWhile(ProcedureState procState, WhileCmd whileCmd, BigBlock parentBB, string runOffTheEndLabel, bool inAtom)
        {
            ...
            VariableSeq varsToHavoc = Util.FilterLocalVariables(Qoogie.CollectAssignedVarsInLoop(body));
            ...
            HavocCmd havocCmd = new HavocCmd(whileCmd.tok, havocExprs);
        ...
        private CmdSeq TranslateCmd(Cmd cmd)
        {
            CmdSeq cmds = new CmdSeq();
            ...
            if (cmd is NewCmd)
            {
                ...
                cmds.Add(new AssumeCmd(Token.NoToken, Expr.Eq(Expr.Select(fieldMapExpr, obj), proofState.deallocExpr)));
                cmds.Add(AssignCmd.MapAssign(Token.NoToken, fieldMapExpr, obj, proofState.allocExpr));
            }
            else if (cmd is FreeCmd)
            {
                ...
                cmds.Add(new AssertCmd(Token.NoToken, Expr.Eq(Expr.Select(fieldMapExpr, obj), proofState.allocExpr)));
                cmds.Add(AssignCmd.MapAssign(Token.NoToken, fieldMapExpr, obj, proofState.deallocExpr));
            } 
            else if (cmd is QHavocCmd)
            {
                QHavocCmd qhavocCmd = cmd as QHavocCmd;
                ...
                foreach (AssignLhs assignLhs in qhavocCmd.Lhss)
                {
                    if (assignLhs is SimpleAssignLhs) 
                    {
                        ...
                        cmds.Add(new HavocCmd(cmd.tok, new IdentifierExprSeq(simpleAssignLhs.DeepAssignedIdentifier)));
                    }
                    else 
                    {
                        ...
                        cmds.Add(new GatedAction(cmd.tok, Expr.True, fcond, new IdentifierExprSeq(mapExpr), false));
                    }
                }
            }
            else if (cmd is GatedAction)
            {
                ...
                {
                    cmds.Add(new AssertCmd(cmd.tok, gact.gate));
                }
                cmds.Add(cmd);
            }
            else
    ...
    public class Qoogie
    {
        ...
        static public BigBlocksResolutionContext ResolveStmt(ProcedureState procState, StmtList stmtList)
        {
            ...
            if (stmt is IfCmd)
            {
                ...
                if (stmt is WhileCmd)
                {
                    ...
                    if (stmt is APLStmt)
                    {
                        ...
                        aplStmt.body.ParentBigBlock = parentBB;
                        aplStmt.body.ParentContext = parentStmt;
                    }
                    else
                    {
                        Debug.Assert(false);
        ...
        public static AtomicStmt MakeAtomicStmt(CmdSeq cmdSeq, StructuredCmd ec, TransferCmd tc)
        {
            ...
            return new AtomicStmt(Token.NoToken, atomLbl, stmtList);
        }

...
procedure {:ispublic true} AQS_acquire(this: NonfairSync, arg: int);
  modifies con, Node_nextWaiter, Node_thread, Node_waitStatus, NonfairSync_head, NonfairSync_tail, owner, lockedBy;
...
implementation Sync_tryRelease(this: NonfairSync, releases: int) returns (result: bool)
{
  ...
  Call_108:
    atomic { // Non-mover
        assert Threads[tid] == this.exclusiveOwnerThread <==> !monitorException;
        assert Threads[tid] == this.exclusiveOwnerThread <==> lockedBy[this] == tid;
        assert releases == 1;
        assert true;
        c := this.state;
        c := c - releases;
  ...
  Call_116:
    atomic { // Non-mover
        assert !monitorException;
        assert Threads[tid] == this.exclusiveOwnerThread;
        assert lockedBy[this] == tid;
        assert true;
        this.state := c;
        if (this.state == 0)
...
implementation Sync_nonfairTryAcquire(this: NonfairSync, acquires: int) returns (result: bool)
{
  ...
  Atomic_117:
    ...
    if (c == 0)
    {
      Call_120:
        atomic { // Non-mover
            ...
            if (dummy)
            {
                assert this.exclusiveOwnerThread == NULL_THREAD;
                assert lockedBy[this] == 0;
                this.state := acquires;
                lockedBy[this] := tid;
            }
        }
    ...
    else
    {
      Call_123:
        atomic { // Non-mover
            assert !monitorException && this.exclusiveOwnerThread != NULL_THREAD;
            assert true;
            ownerThread := this.exclusiveOwnerThread;
        }
...
procedure {:ispublic false} NonfairSync_lock(this: NonfairSync);
  modifies NonfairSync_state, NonfairSync_exclusiveOwnerThread, con, Node_nextWaiter, Node_thread, Node_waitStatus, NonfairSync_head, NonfairSync_tail, owner, lockedBy;
